...
def multi_layer_perceptron(train_data, train_labels, dev_data, dev_labels):
    ...
    # hidden_layer_sizes=(100, ), random_state=0, activation='tanh') .83068
...
def ensemble(mlp_results, knn_results, random_forest_results, dev_labels):
    ...
    Parameters
    ----------
    mpl_resuls: dict[score]: [probability for each class]
        A dictionary containing all of the results from running the multi-layer perceptron classifier 
    knn_results: dict[score]: [probability for each class]
        A dictionary containing all of the results from running the k-nearest neighbors classifier 
    random_forest_results: dict[score]: [probability for each class]
        A dictionary containing all of the results from running the random forest classifier 
    dev_labels: pd.DataFrame
            The dev labels of shape (Dev Examples, 1)
    ...
    predicted_classes: list 
        Predictions for each example for each model
    new_predictions: list 
        New predictions based on a combination of all three models
    """
    # Find max score from each model. best_scores shape: (3,)
    best_scores = [max(mlp_results.keys()),
                   max(knn_results.keys()),
                   max(random_forest_results.keys())]
    # Find maximum probability for each example for each model.
    # prediction_probabilities shape: (3024, 3)
    prediction_probabilities = [np.max(mlp_results[best_scores[0]], axis=1), np.max(
        knn_results[best_scores[1]], axis=1), np.max(random_forest_results[best_scores[2]], axis=1)]
    ...
    predicted_classes = [np.argmax(mlp_results[best_scores[0]], axis=1) +
                         1, np.argmax(knn_results[best_scores[1]], axis=1) +
                         1, np.argmax(random_forest_results[best_scores[2]] +
                                      1, axis=1)]
    ...
    for i, row in enumerate(predicted_classes):
        ...
        # All three models disagree. Choose the label with the highest
        # probability
        else:
            count += 1
    ...
    print(
        "Models disagreed on {0}/{1} dev examples.".format(count, dev_labels.shape[0]))
